...
/// <summary>
/// Кольцевой буфер последних значений тегов: не хранит год, не пишет на диск,
/// даёт последние N точек без await.
///
/// <b>В рантайме сейчас не наполняется — это отладочный/стендовый код.</b>
/// По замыслу (docs/archive-format.md §3, §13.4) кольцо должен наполнять
/// ArchivePipeline, но ветку до него не дотянули: конвейер о кольце не знает,
/// а RuntimeHost создаёт кольцо без таблицы, поэтому подаватель ниже даже
/// не запускается. Живьём кольцо работает только в тестах и бенчмарках,
/// которые конструируют его с таблицей.
///
/// Решено оставить как есть (2026-08-21): корректности оно не добавляет —
/// FileArchiveStore отдаёт и незакрытый блок, поэтому свежий край виден
/// и без кэша, — а выигрыш по скорости никто не мерил. Неархивируемые теги
/// истории не имеют по решению проекта, и в тренд их пускать не следует
/// (проверка перьев при сборке — вместе с C3).
///
/// Если ветку когда-нибудь дотянут, учесть: подаватель ниже не клампит цикл
/// по длине буфера и берёт курсор эпох ПОСЛЕ обработки (теряя записи,
/// пришедшие во время неё), а память считается как 24 байта на точку на тег —
/// 10 000 тегов по 3600 точек это 824 МБ при бюджете АРМа 500 (ТЗ §4.1).
/// </summary>
