...
/// <summary>
/// RuntimeHost: запуск ядра из dev-каталога и из пакета .scadapkg,
/// переходы состояний и доступ к тегам через Client.
...
public class RuntimeHostTests : IDisposable
{
    ...
    [Fact]
    public async Task DevProject_StartReadStop_StateTransitions()
    {
        await using var host = await RuntimeHost.StartAsync(Options(ProjectDir));
    ...
    [Fact]
    public async Task Package_StartReadStop_Works()
    {
