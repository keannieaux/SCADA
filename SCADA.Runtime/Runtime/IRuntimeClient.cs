...
public interface IRuntimeClient : ITagValueReader
{
    ...
    /// <summary>
    /// Теги, изменившиеся после эпохи <paramref name="epoch"/>. Контракт —
    /// как у <see cref="ITagTable.GetChangedSince"/>, там же обоснование:
    /// <b>возвращается полное число изменившихся, а не число записанных
    /// в буфер</b>. Итерироваться — по
    /// <c>Math.Min(результат, destination.Length)</c>; результат больше длины
    /// буфера означает, что часть изменений не видна и нужно пересчитать всё.
    ///
    /// Дополнительно к таблице: клиент сводит проектную и сессионную таблицы
    /// (docs/session-tags-concept.md §4), поэтому изменений за тик бывает
    /// больше, чем тегов в одной из них, а Id приходят глобальные — по ним
    /// нельзя индексировать массив, построенный только по части тегов.
    /// </summary>
